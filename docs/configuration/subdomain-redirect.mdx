...
description: Hide SharaForms's public pages by redirecting users to your main website
---

The `NUXT_PUBLIC_ROOT_REDIRECT_URL` environment variable allows you to "hide" SharaForms's public pages (landing page, integrations page, etc.) by redirecting users to your main website or any other destination when they access certain routes.

## Use Case

This feature is particularly useful when you're running SharaForms on a subdomain (e.g., `forms.yourdomain.com`) and want to:

-   Hide the default SharaForms landing page and branding
...
When `NUXT_PUBLIC_ROOT_REDIRECT_URL` is set, SharaForms will permanently redirect (301) users visiting these routes:
...
-   No exposure to SharaForms branding or interface
