...
description: Detailed guide on configuring environment variables for SharaForms
---

SharaForms uses two `.env` files for configuration: one for the Laravel backend located in the `api` directory, and one for the Nuxt front-end located in the `client` directory.

## Backend Environment Variables

The following environment variables are used to [configure the Laravel](https://laravel.com/docs/11.x/configuration) application (SharaForms's API).
...
| `JWT_TTL`                        | Time to live for JSON Web Tokens (JWT) in minutes. Default: 1440 (24 hours).                                                                                                              |
| `JWT_REMEMBER_TTL`              | JWT time to live when "Remember me" is checked, in minutes. Default: 10080 (7 days).                                                                                                      |
| `SESSION_LIFETIME`              | Session lifetime in minutes. Default: 1440 (24 hours).                                                                                                                                    |
...
| `LICENSE_API_ENDPOINT`           | Cloud API endpoint used by self-hosted instances for Enterprise license validation. Defaults to `https://api.sharaforms.com`.                                                                |
...
Self-hosted Enterprise features are unlocked by activating a license key in the SharaForms UI. Most installations can use the default license API endpoint.
...
OAuth integrations enable user authentication and third-party service connections. SharaForms automatically handles OAuth redirect URLs - you don't need to manually configure redirect URIs in your OAuth applications.
...
| `SHOW_OFFICIAL_TEMPLATES` | Set to `false` to hide official templates from SharaForms's template gallery (defaults to `true`). |
...
SharaForms tracks usage of its products which helps the team to improve and deliver better software. You can opt-out of this telemetry at any time.

**What data is collected?**

None of your customer data is ever transmitted by SharaForms installation. The telemetry data collected are purely usage statistics of various software features:
...
This data will only be used by SharaForms team for:
...
Set `SHARAFORMS_ANONYMOUS_TELEMETRY_DISABLED=true` in your environment variables to disable data collection.
...
| `NUXT_PUBLIC_LICENSE_API_ENDPOINT` | Cloud API endpoint used by the self-hosted client to start Enterprise license checkout. Defaults to `https://api.sharaforms.com`.                                                                                                                                                       |
...
When running SharaForms with Docker, there are some important considerations for handling environment variables:
