...
SharaForms supports OpenID Connect (OIDC) based Single Sign-On (SSO), allowing users to authenticate using their organization's identity provider (IdP) such as Azure Entra ID, Authentik, Okta, or any OIDC-compliant provider.

<Info>
    SharaForms handles OIDC redirect URLs automatically. You don't need to manually
...
<Step title="Configure Field Mappings (Optional)">
    ...
    <Note>
        If field mappings are not configured, SharaForms will use standard OIDC claim names (`email`, `name`, etc.).
    </Note>
...
SharaForms is compatible with any OIDC-compliant identity provider, including:
...
SharaForms generates and validates an OIDC `state` value by default for login redirects. Callback requests without a valid, unexpired state are rejected to prevent login CSRF.
...
For security, SharaForms prevents automatic linking of existing accounts. If a user with the same email already exists but isn't linked to the OIDC connection, authentication will fail with a message to contact the administrator.
...
-   **Regular Updates**: Keep your IdP configuration in sync with SharaForms connection settings
