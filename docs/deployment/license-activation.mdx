...
description: Activate a self-hosted Enterprise license on your SharaForms installation
---

Use a self-hosted Enterprise license key to unlock Enterprise features and add more than 2 users on your SharaForms instance.
...
-   A running self-hosted SharaForms installation
-   A user account that is an admin of at least one workspace
-   A self-hosted Enterprise license key
-   Network access from your API container or server to the SharaForms license API

By default, self-hosted license validation uses `https://api.sharaforms.com`.
...
<Step title="Activate the license">
    Click **Activate License**. When activation succeeds, SharaForms refreshes the instance feature flags and enables the licensed Enterprise features.
</Step>
</Steps>

## License-related environment variables

Most installations can use the default license API settings. Override these variables only when instructed by SharaForms support or when you run a custom license API endpoint.

| Variable | Description |
| -------- | ----------- |
| `LICENSE_API_ENDPOINT` | Backend license API endpoint. Defaults to `https://api.sharaforms.com`. |
...
| `NUXT_PUBLIC_LICENSE_API_ENDPOINT` | Front-end license API endpoint used to start Enterprise license checkout. Defaults to `https://api.sharaforms.com`. |
...
SharaForms caches license checks. If the license API is temporarily unavailable after a successful check, the instance can enter a short grace state and continue using licensed features while it retries validation.
...
<Accordion title="The license key is invalid or expired">
    Confirm that you copied the full key and that the license is active. If the key still fails, contact SharaForms support.
</Accordion>
