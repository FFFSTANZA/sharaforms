...
description: "Deploy SharaForms using Docker"
---
...
<Tip>
    Looking to develop SharaForms locally? Check out our [Docker Development
...
1. Clone the repository:
    ```bash
    git clone https://github.com/SharaForms/SharaForms.git
    cd SharaForms
    ```
...
3. Access your SharaForms instance at `http://localhost`

### Initial Setup

After deployment, SharaForms will automatically redirect you to a setup page where you can create your admin account. Simply visit `http://localhost` and you'll be guided through the setup process.
...
<Info>
    A self-hosted Enterprise license is optional. Core self-hosted SharaForms works
...
SharaForms provides pre-built Docker images for easy deployment:

-   [SharaForms API Image](https://hub.docker.com/r/sharaforms/api)
-   [SharaForms Client Image](https://hub.docker.com/r/sharaforms/ui)
...
docker build -t sharaforms/api:local -f docker/Dockerfile.api .
docker build -t sharaforms/ui:local -f docker/Dockerfile.client .
```
...
services:
    api:
        image: sharaforms/api:local
        environment:
            PHP_MEMORY_LIMIT: 1G
    ui:
        image: sharaforms/ui:local
    ingress:
        volumes:
