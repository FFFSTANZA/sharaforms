---
title: Multiple Workspaces and Team Roles
description: Use Enterprise multi-workspace and team role controls on self-hosted SharaForms
---

Self-hosted Enterprise can unlock workspace and team administration features for organizations that manage several teams, clients, or departments in one SharaForms instance.

<Info>
    Self-hosted Community instances are limited to 2 users total across the
    whole instance. A self-hosted Enterprise license is required to add more
    than 2 users, whether users are invited manually or provisioned through
    OIDC.
</Info>

## What this unlocks

Depending on the features included in your license, multi-org access can unlock:

-   Multiple workspaces
-   More than 2 users across the instance
-   Advanced workspace member roles
-   Higher-level organization controls around workspace administration

## Before you start

Make sure you have:

-   An active self-hosted Enterprise license with multi-org features enabled
-   Workspace admin access
-   A clear workspace structure for your teams or clients

## Related pages

-   [Self-hosted License](/deployment/self-hosted-license)
-   [License Activation](/deployment/license-activation)
-   [Workspace Users API](/api-reference/workspace-users/list-workspace-users)
