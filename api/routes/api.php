...
Route::get('/healthcheck', [HealthCheckController::class, 'apiCheck'])->name('healthcheck');
...
Route::group(['middleware' => 'auth.multi'], function () {
    ...
    Route::get('user/export', [UserController::class, 'exportPersonalData'])->name('user.export');
    ...
    Route::prefix('subscription')->name('subscription.')->group(function () {
        Route::put('/update-customer-details', [SubscriptionController::class, 'updateCustomerDetails'])->name('update-customer-details');
        Route::get('/new/{subscription}/{plan}/checkout', [SubscriptionController::class, 'checkout'])
...
Route::group(['middleware' => 'guest:api'], function () {
    Route::post('login', [LoginController::class, 'login'])->middleware('throttle:auth-login')->name('login');
    Route::post('register', [RegisterController::class, 'register'])->middleware('throttle:auth-register');

    Route::post('password/email', [ForgotPasswordController::class, 'sendResetLinkEmail'])->middleware('throttle:password-email');
    Route::post('password/reset', [ResetPasswordController::class, 'reset'])->middleware('throttle:password-reset');

    Route::post('email/verify/{user}', [VerificationController::class, 'verify'])->middleware('throttle:email-verify')->name('verification.verify');
    Route::post('email/resend', [VerificationController::class, 'resend'])->middleware('throttle:email-resend');

    // OIDC email lookup endpoint (for login flow)
    Route::post('auth/oidc/options', [\App\Http\Controllers\Auth\SsoController::class, 'getOptionsForEmail'])->middleware('throttle:auth-oidc-options')->name('sso.options');

    // Two-factor authentication verification (public, but requires pending auth token)
    Route::post('/auth/two-factor/verify', [\App\Http\Controllers\Auth\TwoFactorVerificationController::class, 'verify'])->middleware('throttle:auth-two-factor')->name('two-factor.verify');
});
...
Route::prefix('forms')->name('forms.')->group(function () {
    ...
    // Schedule status
    Route::get('{form}/schedule-status', [PublicFormController::class, 'scheduleStatus'])->name('schedule-status');
...
Route::post(
    '/dodo/webhook',
    [\App\Http\Controllers\Webhook\DodoPaymentsController::class, 'handle']
)->name('dodo.webhook');
