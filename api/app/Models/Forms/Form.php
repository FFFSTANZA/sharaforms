...
class Form extends Model implements CachableAttributes, VersionableNestedDiff
{
    ...
    public const EMBED_TYPES = ['inline', 'popup', 'slide-in', 'bubble'];
    ...
    {
        $this->attributes['opens_at'] = ($value) ? Carbon::parse($value)->setTimezone('UTC') : null;
    }
    ...
    public function getIsNotYetOpenAttribute()
    {
        // If opens_at is in the future, form is not yet open
        if ($this->opens_at && now()->lt($this->opens_at)) {
            return true;
        }

        // If schedule exists and we're outside the recurring window, not open
        if ($this->schedule !== null && !$this->isWithinRecurringSchedule()) {
            return true;
        }
    ...
    {
        return $this->opens_at !== null || $this->schedule !== null;
    }
    ...
    public function getScheduleStatusAttribute()
    {
        ...
        if ($this->schedule && $this->isWithinRecurringSchedule()) {
            return 'open';
        }

        if ($this->schedule && !$this->isWithinRecurringSchedule()) {
            return 'closed_by_schedule';
        }

        if ($this->opens_at && $now->gte($this->opens_at)) {
            return 'open';
        }
    ...
    public function isWithinRecurringSchedule(): bool
    {
        if ($this->schedule === null) {
            return true; // No recurring schedule = always within schedule
        }

        if (empty($this->schedule['windows'])) {
            return false; // Schedule enabled but no windows = never available
        }

        $now = now();
        $timezone = $this->schedule['timezone'] ?? 'UTC';
        $nowTz = Carbon::parse($now)->setTimezone($timezone);

        $dayOfWeek = strtolower($nowTz->dayName);
        $currentTime = $nowTz->format('H:i');

        foreach ($this->schedule['windows'] as $window) {
            $days = array_map('strtolower', $window['days'] ?? []);
            if (!in_array($dayOfWeek, $days, true)) {
                continue;
            }

            $startTime = $window['start_time'] ?? '00:00';
            $endTime = $window['end_time'] ?? '23:59';
