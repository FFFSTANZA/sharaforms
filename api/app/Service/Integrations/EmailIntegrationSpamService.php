...
class EmailIntegrationSpamService
{
    ...
    public function checkForSpam(Form $form, FormIntegration $integration): ?array
    {
        ...
        if (!$this->hasAiProvider($form, $integration)) {
            return null;
        }
    ...
    private function shouldCheck(Form $form, FormIntegration $integration): bool
    {
        ...
        // Skip if the integration is not configured with a target address yet.
        $data = (array) ($integration->data ?? (object) []);
        if (!$data || empty($data['send_to'])) {
            return false;
        }
    ...
    private function hasAiProvider(Form $form, FormIntegration $integration): bool
    {
        if (AiProviderManager::hasAvailableProvider()) {
            return true;
        }

        Log::warning('Skipping email integration spam check because no supported AI provider API key is configured.', [
            'form_id' => $form->id,
            'integration_id' => $integration->id,
            'user_id' => $form->creator?->id,
        ]);
