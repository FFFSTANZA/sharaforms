...
class FormCleaner
{
    ...
    private array $cleaningMessages = [
        // For form
        'no_branding' => 'SharaForms branding is not hidden.',
        ...
        'form_scheduling' => 'Form scheduling was disabled',
        'embed' => 'Advanced embed types were disabled',
    ...
    public function processRequest(UserFormRequest $request): FormCleaner
    {
        ...
        $allowOnSelfHosted = config('app.self_hosted', true) && (bool) config('sharaforms.custom_code.enable_self_hosted', false);
    ...
    private function cleanForTier(Workspace $workspace, array $data, bool $simulation = false): array
    {
        ...
        foreach (array_keys($formFeatures) as $feature) {
            if (!$planAccessService->hasFormFeature($workspace, $feature)) {
                if ($feature === 'embed') {
                    $embedType = $data['embed_type'] ?? 'inline';
                    if ($embedType !== 'inline') {
                        if (!isset($this->cleanings['form'])) {
                            $this->cleanings['form'] = [];
                        }
                        $this->cleanings['form'][] = 'embed';
                        if (!$simulation) {
                            $data['embed_type'] = 'inline';
                            $data['embed_settings'] = [];
                        }
                    }
                    continue;
                ...
                if ($feature === 'form_scheduling') {
                    $opensAtVal = $data['opens_at'] ?? null;
                    $scheduleVal = $data['schedule'] ?? [];
                    if ($opensAtVal !== null || !empty($scheduleVal)) {
                        if (!isset($this->cleanings['form'])) {
                            $this->cleanings['form'] = [];
                        }
                        $this->cleanings['form'][] = 'form_scheduling';
                        if (!$simulation) {
                            $data['opens_at'] = null;
                            $data['schedule'] = [];
                        }
