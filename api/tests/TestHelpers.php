...
trait TestHelpers
{
     ...
     */
    /**
     * Enable Dodo pricing in tests by setting up required config.
     */
    public function enablePricing(): void
    {
        config()->set('dodo.api_key', 'test_key');
        config()->set('dodo.products', [
            'pro' => ['monthly' => 'prod_pro_monthly', 'yearly' => 'prod_pro_yearly'],
            'business' => ['monthly' => 'prod_business_monthly', 'yearly' => 'prod_business_yearly'],
            'enterprise' => ['monthly' => 'prod_enterprise_monthly', 'yearly' => 'prod_enterprise_yearly'],
        ]);
    }

    /**
     * Disable Dodo pricing in tests (restore default state).
     */
    public function disablePricing(): void
    {
    ...
    public function actingAsAdmin(?User $user = null)
    {
        if ($user == null) {
            ...
            config(['sharaforms.admin_emails' => [$adminEmail]]);
