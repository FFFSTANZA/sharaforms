...
it('skips ai email integration spam checks when the openai key is missing', function () {
    config()->set('sharaforms.admin_emails', []);
    config()->set('sharaforms.moderator_emails', []);
...
it('considers non-self-targeting email integrations eligible for spam checks', function () {
    config()->set('spam.enabled', true);
    ...
    $integration = (new FormIntegration())->forceFill([
        ...
        'status' => FormIntegration::STATUS_ACTIVE,
        'data' => (object) [
            'send_to' => 'third-party@example.test',
            'subject' => 'Secure sign in required',
            'email_content' => 'Please sign in to continue.',
        ],
    ]);

    $service = new EmailIntegrationSpamService(Mockery::mock(UserActionService::class));
    $method = new ReflectionMethod(EmailIntegrationSpamService::class, 'shouldCheck');
    $method->setAccessible(true);

    expect($method->invoke($service, $form, $integration))->toBeTrue();
});
