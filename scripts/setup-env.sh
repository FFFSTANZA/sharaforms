...
# Welcome to the SharaForms environment setup script!
...
for arg in "$@"; do
  if [ "$arg" == "--docker" ]; then
    ...
    echo "SharaForms setup detected the --docker flag. Preparing Docker-specific environment..."
    break
  fi
done
...
if [ -f "$ENV_FILE" ]; then
  echo "SharaForms's main .env file is already in place. No further action is needed."
else
  echo "Creating SharaForms's main .env file from the template..."
  cp "$ENV_EXAMPLE" "$ENV_FILE"

  # Secure your SharaForms instance with a unique APP_KEY
...
if [ -f "$CLIENT_ENV_FILE" ]; then
  echo "SharaForms's client .env file is already configured. Moving on..."
else
  echo "Creating SharaForms's client .env file from the template..."
  cp "$CLIENT_ENV_EXAMPLE" "$CLIENT_ENV_FILE"
  set_env_value "$CLIENT_ENV_FILE" "NUXT_API_SECRET" "$SHARED_SECRET"
fi

echo "✅ SharaForms environment setup is now complete. Enjoy building your forms!"