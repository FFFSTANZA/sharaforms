...
function withSlideMedia(field, media, layout = "right-split", extra = {}) {
  return {
    ...
    image: {
      ...
      media_key: media,
      alt: "Abstract SharaForms live demo visual",
      layout,
...
function introSlide(id, title, body, layout = "right-split", media = "intro") {
  return withSlideMedia(
    textBlock(
      id,
      `<p><strong style="background: linear-gradient(135deg, #ff8a5b, #ef5da8, #8b5cf6, #3b82f6); -webkit-background-clip: text; -webkit-text-fill-color: transparent; background-clip: text;">Live demo</strong></p><h2>${title}</h2><p>${body}</p>`,
    ),
    media,
    layout,
  )
...
function urlField(id, name, extra = {}) {
  return {
    ...
    required: false,
    hidden: false,
    placeholder: "https://${competitorSlug(competitorName",
    ...extra,
  }
}
...
function baseScenario(key, overrides) {
  return {
    key,
    eyebrow: "Live demo",
    title: "Try SharaForms live",
    description: "Answer a short sample form and see how SharaForms feels for respondents.",
    urlLabel: "sharaforms.com/live-demo",
    ...
    successBody: "You just tried a real SharaForms flow: visuals, fields, conditional logic, and a custom ending in one simple form.",
    form: baseForm(key),
    ...overrides,
  }
}
...
function buildHomeScenario() {
  const form = baseForm("home", {
    ...
    properties: [
      introSlide(
        "home_intro",
        '<span style="font-family: euclid-circular-b, sans-serif; font-weight: 600; background: linear-gradient(135deg, #ff8a5b, #ef5da8, #8b5cf6, #3b82f6); -webkit-background-clip: text; -webkit-text-fill-color: transparent; background-clip: text;">See SharaForms in action</span>',
      ...
      withSlideMedia(
        textBlock(
          "home_summary",
          `<p><strong>Demo complete</strong></p><h2><span style="background: linear-gradient(135deg, #ff8a5b, #ef5da8, #8b5cf6, #3b82f6); -webkit-background-clip: text; -webkit-text-fill-color: transparent; background-clip: text;">Thanks, ${mention("home_name", "Name", "Alex")}.</span></h2><p>You rated the demo ${mention("home_rating", "Rating", "5")}/5 and picked ${mention("home_use_case", "Use case", "a form workflow")}.</p><p>Your note: ${mention("home_comment", "Comment", "a short answer")}</p>`,
        ),
        "summary",
        "right-split",
      ),
  ...
  return baseScenario("home", {
    title: "Try a real SharaForms live.",
...
function comparisonScenario(key, competitorName, importSource, overrides) {
  return baseScenario(`comparison-${key}`, {
    urlLabel: `sharaforms.com/vs-${competitorSlug(competitorName)}`,
...
function buildTypeformScenario(importSource) {
  ...
  const properties = [
    introSlide(
      "typeform_intro",
      '<span class="text-blue-500">Build a Typeform-style form in SharaForms.</span>',
    ...
    withSlideMedia(
      textBlock(
        "typeform_summary",
        `<p><strong>Your demo form</strong></p><h2><span class="text-blue-500">A Typeform-style form, built in SharaForms.</span></h2><p>${mention("typeform_name", "Name", "Jamie")} wants to collect ${mention("typeform_use_case", "Use case", "inbound leads")} and then ${mention("typeform_controls", "Follow-up actions", "send responses to a webhook")}.</p>`,
      ),
      "summary",
      "right-split",
    ),
  ...
  return comparisonScenario("typeform", competitorName, importSource || "typeform", {
    title: "Try a Typeform-style flow in SharaForms.",
    ...
    successBody: "That was a Typeform-style experience, but with SharaForms's routing and unlimited responses behind it.",
    form,
  })
}
...
function buildGoogleFormsScenario(importSource) {
  ...
  const properties = [
    textBlock(
      "google_forms_intro",
      "<p><strong>Live demo</strong></p><h2>Make a Google Forms-style request form feel polished.</h2><p>Answer a few simple questions. SharaForms turns them into a clean form with follow-up actions.</p>",
    ),
  ...
  return comparisonScenario("google-forms", competitorName, importSource || "google_forms", {
    ...
    successBody: "That was a Google Forms-style request upgraded into a branded SharaForms form.",
    form,
  })
}
...
function buildTallyScenario(importSource) {
  ...
  const properties = [
    introSlide(
      "tally_intro",
      '<span class="text-blue-500">Build a simple Tally-style form in SharaForms.</span>',
  ...
  return comparisonScenario("tally", competitorName, importSource || "tally", {
    ...
    successBody: "That was a simple Tally-style flow with SharaForms controls ready when the project grows.",
    form,
  })
}
...
function buildJotformScenario(importSource) {
  ...
  return comparisonScenario("jotform", competitorName, importSource, {
    ...
    successBody: "That was a Jotform-style intake workflow rebuilt around SharaForms's scaling story.",
    form,
  })
}
...
function buildFilloutScenario(importSource) {
  ...
  const properties = [
    textBlock(
      "fillout_intro",
      "<p><strong>Live demo</strong></p><h2>Build a lead form that sends people to the right next step.</h2><p>This demo shows a simple workflow with routing, exports, webhooks, and flexibility.</p>",
    ),
    ...
    checkboxField("fillout_open_source", "I want a self-hosted fallback if this workflow grows", {
      prefill: true,
    }),
  ...
  return comparisonScenario("fillout", competitorName, importSource, {
    ...
    successBody: "That was a simple lead workflow using SharaForms's routing, exports, and flexibility.",
    form,
  })
}
...
function buildHeyformScenario(importSource) {
  ...
  return comparisonScenario("heyform", competitorName, importSource, {
    ...
    successBody: "That was a HeyForm-style conversational flow with SharaForms workflow depth built in.",
    form,
  })
}
...
function buildYouformScenario(importSource) {
  ...
  return comparisonScenario("youform", competitorName, importSource, {
    ...
    description: "A focused lightweight form that shows when SharaForms's logic, embeds, and ownership become useful.",
    highlights: ["Focused mode", "Multi-select", "Platform score"],
    successBody: "That was a Youform-style simple flow with SharaForms's path to richer workflows.",
    form,
  })
}
...
function buildFormbricksScenario(importSource) {
  ...
  const properties = [
    introSlide(
      ...
      "Formbricks is strong for feedback. This SharaForms demo shows what happens when the job expands into applications, registrations, and lead capture.",
      "right-split",
      "intro",
    ),
    withSlideMedia(
  ...
  return comparisonScenario("formbricks", competitorName, importSource, {
    ...
    description: "A focused flow that contrasts feedback collection with broader SharaForms use cases and routing.",
    highlights: ["Focused mode", "Use-case branching", "Workflow score"],
    successBody: "That was an SharaForms workflow tailored to someone comparing Formbricks for more than surveys.",
    form,
  })
}
...
function buildFormioScenario(importSource) {
  ...
  return comparisonScenario("formio", competitorName, importSource, {
    ...
    successBody: "That was a Form.io-style production intake reframed around SharaForms's no-backend setup.",
    form,
  })
}
...
function build123FormBuilderScenario(importSource) {
  ...
  return comparisonScenario("123formbuilder", competitorName, importSource, {
    ...
    successBody: "That was a 123FormBuilder-style request form showing why SharaForms fits growing workflows.",
    form,
  })
}
...
function buildComparisonScenario(competitorName, importSource) {
  ...
  const properties = [
    introSlide(
      "cmp_intro",
      '<span class="text-blue-500">Try the SharaForms switching workflow.</span>',
      `A short live form for people comparing ${safeName} with SharaForms.`,
      "right-split",
      "switch",
    ),
    withSlideMedia(
  ...
  return comparisonScenario(competitorSlug(safeName), safeName, importSource, {
    title: "Try a switching workflow in SharaForms.",
    description: `Answer a short live form for people comparing ${safeName} with SharaForms.`,
    highlights: ["Focused mode", "Conditional import", "Switch score"],
    successBody: `That was an SharaForms switching workflow tailored to someone comparing ${safeName}.`,
    form,
  })
}
