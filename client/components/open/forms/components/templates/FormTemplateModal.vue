  ...
  >
    <template #header>
      ...
      <UButton
        ...
        size="sm"
        @click="crisp.openHelpdeskArticle('how-to-create-a-sharaforms-template-1fn84i4')"
      >
        Help
      </UButton>
    </template>
...
function initForm(tpl) {
  const data = tpl && Object.keys(tpl).length ? tpl : defaults
  templateForm.value = useForm(data)
}

onMounted(() => initForm(props.template))
