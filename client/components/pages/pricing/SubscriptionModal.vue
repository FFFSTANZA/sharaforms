              ...
              <div class="rounded-[1.75rem] border border-slate-200 bg-white/90 p-5 shadow-sm sm:p-6">
                <div class="flex items-center gap-3">
                  <div class="flex h-11 w-11 items-center justify-center rounded-2xl bg-slate-950 text-white shadow-sm">
                    <UIcon name="lucide:star" class="h-5 w-5" />
                  </div>
                  <div>
...
const PLAN_DETAILS = {
  pro: {
    ...
    highlights: [
      'Remove SharaForms branding',
      'Use one custom domain',
      'Unlock analytics, summaries, and premium integrations',
    ],
    features: [
      { icon: 'lucide:star', title: 'Remove SharaForms branding', description: 'Publish forms without the SharaForms watermark and make the experience feel truly yours.' },
      { icon: 'lucide:globe', title: '1 custom domain', description: 'Put forms on your own domain for a polished, trustworthy handoff.' },
      { icon: 'lucide:bell', title: 'Pro integrations', description: 'Send alerts to Slack, Discord, Telegram, and unlock the rest of the Pro workflow tools.' },
    ],
  },
  business: {
    ...
    features: [
      { icon: 'lucide:users', title: 'Roles for your team', description: 'Invite teammates with the right level of access instead of giving everyone the same permissions.' },
      { icon: 'lucide:paintbrush', title: 'Advanced branding', description: 'Use custom CSS, fonts, and richer styling controls to match your product or campaign.' },
      { icon: 'lucide:clipboard-check', title: 'Operational submission flows', description: 'Unlock partial submissions and team-oriented controls for more complex forms.' },
    ],
  },
  enterprise: {
    ...
    features: [
      { icon: 'lucide:shield-check', title: 'Enterprise SSO', description: 'Connect SAML or LDAP so access is controlled from your identity provider.' },
      { icon: 'lucide:file-text', title: 'Audit logs & compliance', description: 'Track important activity and support teams that need stronger internal controls.' },
      { icon: 'lucide:server', title: 'External storage', description: 'Route storage to your own infrastructure when you need tighter operational ownership.' },
    ],
  },
  self_hosted: {
    subtitle: 'Security, compliance, and identity controls for teams that need centralized governance.',
    ...
    features: [
      { icon: 'lucide:globe', title: 'Remove SharaForms branding', description: 'Publish forms without the SharaForms watermark and make the experience feel truly yours.' },
      { icon: 'lucide:shield-check', title: 'Enterprise SSO', description: 'Connect SAML or LDAP and add more than 2 users on your self-hosted instance.' },
      { icon: 'lucide:file-text', title: 'Audit logs & compliance', description: 'Track important activity and support teams that need stronger internal controls.' },
    ],
  },
}
...
const props = defineProps({
  ...
  modal_description: {
    type: String,
    default: 'Unlock all features and get the most out of SharaForms.'
  },
  plan: {
    type: String,
    default: 'pro'
...
const { startCheckout: openBillingCheckout, isLoading: checkoutLoading, isPlanLoading } = useBillingCheckout()
...
const modalDescription = computed(() => {
  if (props.modal_description && props.modal_description !== 'Unlock all features and get the most out of SharaForms.') {
  ...
  const descriptions = {
    ...
    self_hosted: 'Unlock all features and get the most out of SharaForms.',
  }
...
const getPlanCardClasses = (planOption) => {
  ...
  return [
    planOption.isRequired
      ? `${PLAN_VISUALS[planOption.key].selectedCardClass} scale-[1.01]`
...
const startCheckout = async (planName) => {
  ...
  try {
    await openBillingCheckout(planName, {
