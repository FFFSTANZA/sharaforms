  ...
  <div>
    <template v-if="displayAll">
      <span
        ...
        class="inline-flex items-center gap-1 px-2 py-1 text-xs font-medium text-white brand-gradient-warm rounded-full"
      >
        <svg
          aria-hidden="true"
          class="w-3 h-3"
      ...
      <span
        ...
        :key="item.slug"
        class="brand-chip-strong inline-flex items-center rounded-full px-2 py-1 text-xs font-medium"
      >
        {{ item.name }}
      </span>
    </template>
