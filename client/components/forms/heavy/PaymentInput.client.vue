...
const setAccountLoadingError = computed(() => stripeElements.value?.setAccountLoadingError)
const setIntentId = computed(() => stripeElements.value?.setIntentId)
...
onMounted(async () => {
  try {
    ...
    if (compVal.value && isPaymentIntentId(compVal.value) && setIntentId.value) {
      console.debug('[PaymentInput] Syncing existing payment intent:', compVal.value)
      setIntentId.value(compVal.value)
    }
    ...
    if (setAccountLoadingError.value) {
      setAccountLoadingError.value('Failed to initialize Stripe. Please refresh and try again.')
    }
...
const onStripeError = (error) => {
  ...
  if (setAccountLoadingError.value) {
    setAccountLoadingError.value(errorMessage + '. Please check configuration or refresh.')
  }
}
