/* eslint-disable */
/**
 * SharaForms SDK Bridge & Local SDK
  ...
  emit(event, data) {
    if (this._listeners[event]) {
      this._listeners[event].forEach(callback => {
        try { callback(data) } catch (e) { console.error('[SharaForms SDK] Error in event handler:', e) }
      })
    }
    if (this._onceListeners[event]) {
      ...
      callbacks.forEach(callback => {
        try { callback(data) } catch (e) { console.error('[SharaForms SDK] Error in once handler:', e) }
      })
    }
  }
}
...
class LocalFormInstance extends EventEmitter {
  ...
  _registerBridge(bridgeData) {
    ...
    if (typeof window !== 'undefined' && window.sharaforms?._isStub) {
      const formStub = window.sharaforms._forms?.[this.slug]
      if (formStub) {
  ...
  setTheme(theme) { 
    console.warn('[SharaForms SDK] setTheme is not yet implemented')
    return false
  }
  ...
  open() { console.warn('[SharaForms SDK] open() not applicable for non-iframe forms') }
  close() { console.warn('[SharaForms SDK] close() not applicable for non-iframe forms') }
  toggle() { console.warn('[SharaForms SDK] toggle() not applicable for non-iframe forms') }
  isOpen() { return true }
...
/**
 * Local SharaForms SDK - runs on form pages directly
 */
class SharaFormsLocalSDK extends EventEmitter {
  constructor() {
    super()
    this._forms = {}
  ...
  create(slug, options = {}) {
    console.warn('[SharaForms SDK] create() not applicable for local SDK.')
    return null
  }
}
...
export function createLocalSDK() {
  ...
  if (window.sharaforms && window.sharaforms._isLocal) {
    return window.sharaforms
  }

  const sdk = new SharaFormsLocalSDK()
  sdk._isLocal = true
  
  // Migrate from stub if exists
  if (window.sharaforms && window.sharaforms._isStub) {
    const stub = window.sharaforms
...
function initLocalSDK() {
  ...
  if (window.sharaforms && !window.sharaforms._isStub) return window.sharaforms
  
  const sdk = createLocalSDK()
  if (sdk) {
    window.sharaforms = sdk
  }
  return sdk
}
...
export function useSdkBridge(options) {
  ...
  function postMessageSafe(target, message, origin) {
    try {
      ...
      if (error?.name !== 'DataCloneError') {
        console.error('[SharaForms SDK Bridge] postMessage failed:', error)
      }
    }
  }
  ...
  function toPlainObject(obj) {
    ...
    try {
      ...
      console.warn('[SharaForms SDK] Failed to serialize payload:', e)
      return {}
    }
  }
  ...
  function handleCommand(message, respond = sendResponse) {
    ...
    try {
      ...
      console.error('[SharaForms SDK Bridge] Command error:', e)
...
export { SharaFormsLocalSDK, EVENTS }
