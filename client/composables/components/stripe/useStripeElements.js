...
export const createStripeElements = (initialAccountId = null) => {
  ...
  /**
   * Sets an account loading error state (used by PaymentInput for config/prep errors)
   * @param {String} message - Error message to display
   */
  const setAccountLoadingError = (message) => {
    state.hasAccountLoadingError = true
    state.errorMessage = message
  }

  /**
   * Clears the account loading error state
   */
  const clearAccountLoadingError = () => {
    state.hasAccountLoadingError = false
    state.errorMessage = ''
  }
