/* eslint-disable */
/**
 * SharaForms JavaScript SDK
 * Enables programmatic interaction with embedded SharaForms forms
...
(function (global) {
  ...
  class FormInstance extends EventEmitter {
    ...
    _sendCommand(command, payload = {}) {
      if (!this.iframe || !this.iframe.contentWindow) {
        console.warn('[SharaForms SDK] Cannot send command - iframe not available')
   ...
   * Main SharaForms SDK
   */
  class SharaFormsSDK extends EventEmitter {
    constructor() {
      super()
      this._forms = {}
      this._options = {
    ...
    create(slug, options = {}) {
      ...
      if (!containerEl) {
        console.error('[SharaForms SDK] Container element not found')
        return null
      }
  ...
  const sharaforms = new SharaFormsSDK()

  // Expose to global scope
  global.sharaforms = sharaforms

  // Backward compatibility with initEmbed
  if (!global.initEmbed) {
    global.initEmbed = function(formSlug, options = {}) {
      sharaforms.init(options)
      
      const form = sharaforms.get(formSlug)
      if (form) {
        form.on('submit', (data) => {
      ...
      if (options.autoResize !== false) {
        sharaforms.initAutoResize(formSlug)
      }
    }
  }
