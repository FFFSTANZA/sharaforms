<template>
  <div class="flex flex-col min-h-full">
    ...
    <template v-else>
      ...
      <section
        v-if="industry.what_to_include && industry.what_to_include.length"
        ...
        <div class="px-4 mx-auto sm:px-6 lg:px-8 max-w-7xl">
          <div class="mx-auto max-w-4xl">
            <h2 class="text-xl font-bold tracking-tight text-neutral-900 dark:text-neutral-50 sm:text-2xl">
              Common {{ industry.name.toLowerCase() }} every team should have
            </h2>
            <ul class="mt-6 grid grid-cols-1 gap-x-8 gap-y-3 sm:grid-cols-2">
              <li
                v-for="(item, itemKey) in industry.what_to_include"
                :key="itemKey"
      ...
      <!-- START HOW TO -->
      <section
        v-if="industry.how_to_steps && industry.how_to_steps.length"
        class="bg-neutral-50 py-14 sm:py-20 dark:bg-neutral-950"
      >
        <div class="px-4 mx-auto sm:px-6 lg:px-8 max-w-7xl">
          <div class="mx-auto max-w-4xl">
            <h2 class="text-xl font-bold tracking-tight text-neutral-900 dark:text-neutral-50 sm:text-2xl">
              How to build your {{ industry.name.toLowerCase().replace(/ forms$/, '') }} form workflow in SharaForms
            </h2>
            <ol class="mt-8 space-y-6">
              <li
                v-for="(step, stepKey) in industry.how_to_steps"
                :key="stepKey"
      ...
      <!-- START PRACTICES -->
      <section
        v-if="(industry.best_practices && industry.best_practices.length) || (industry.common_mistakes && industry.common_mistakes.length)"
      ...
      >
        <div class="px-4 mx-auto sm:px-6 lg:px-8 max-w-7xl">
          <div class="mx-auto grid max-w-5xl grid-cols-1 gap-10 md:grid-cols-2 md:gap-x-12">
            <div
              v-if="industry.best_practices && industry.best_practices.length"
            >
              <h2 class="text-lg font-bold tracking-tight text-neutral-900 dark:text-neutral-50 sm:text-xl">
                Best practices for {{ industry.name.toLowerCase() }}
              </h2>
              <ul class="mt-5 space-y-3">
                <li
                  v-for="(item, itemKey) in industry.best_practices"
                  :key="'bp-' + itemKey"
                  class="flex items-start gap-3 text-sm leading-6 font-normal text-neutral-600 dark:text-neutral-300"
                >
                  <span class="mt-0.5 inline-flex h-5 w-5 shrink-0 items-center justify-center rounded-full brand-gradient-warm">
            ...
            <div
              v-if="industry.common_mistakes && industry.common_mistakes.length"
            >
              <h2 class="text-lg font-bold tracking-tight text-neutral-900 dark:text-neutral-50 sm:text-xl">
              ...
              <ul class="mt-5 space-y-3">
                <li
                  v-for="(item, itemKey) in industry.common_mistakes"
                  :key="'cm-' + itemKey"
                  class="flex items-start gap-3 text-sm leading-6 font-normal text-neutral-600 dark:text-neutral-300"
                >
      ...
      <!-- START FAQ -->
      <section
        v-if="industry.faqs && industry.faqs.length"
        class="bg-white border-t border-neutral-200 py-14 sm:py-20 dark:border-neutral-800"
      >
        <div class="px-4 mx-auto sm:px-6 lg:px-8 max-w-7xl">
          <div class="mx-auto max-w-3xl">
            <h2 class="text-xl font-bold tracking-tight text-neutral-900 dark:text-neutral-50 sm:text-2xl">
              Frequently asked questions about {{ industry.name.toLowerCase() }}
            </h2>
            <dl class="mt-8 space-y-8">
              <div
                v-for="(faq, faqKey) in industry.faqs"
                :key="faqKey"
              >
...
if (!industry.value) {
  throw createError({ statusCode: 404, statusMessage: 'Template industry page not found' })
}
...
const featuredTemplate = computed(() => {
  if (!industry.value?.featured_template || !allTemplates.value) return null
  return allTemplates.value.find(
    (t) => t.slug === industry.value.featured_template,
  ) || null
})
...
useOpnSeoMeta({
  ...
  ogImage: () =>
    industry.value
      ? `/og/industry/${encodeURIComponent(route.params.slug)}?title=${encodeURIComponent(industry.value.h1 || industry.value.name)}&desc=${encodeURIComponent((industry.value.meta_description || '').slice(0, 120))}`
      : '/share-preview.jpg',
  speakable: ["h1", "p"],
  breadcrumbs: [
    { name: "Home", item: "/" },
  ...
  script: [
    {
      ...
      textContent: JSON.stringify({
        ...
        keywords: `free form templates, ${industry.value?.name || 'industry'} form templates, free form builder, unlimited submissions, built-in calculations, no-code form templates`,
        isPartOf: {
    ...
    ...(industry.value?.faqs && industry.value.faqs.length
      ? [{
          key: `template-industry-faq-schema:${route.params.slug}`,
          type: 'application/ld+json',
          textContent: JSON.stringify({
            '@context': 'https://schema.org',
            '@type': 'FAQPage',
            mainEntity: industry.value.faqs.map((faq) => ({
              '@type': 'Question',
              name: faq.question,
              acceptedAnswer: {
                '@type': 'Answer',
